...
export async function reserveBrowserRun(
  ...
  try {
    return await storage.transaction(async (transaction) => {
      const stored = await transaction.get([BROWSER_QUOTA_DATE_KEY, BROWSER_QUOTA_TOTAL_KEY]);
      ...
      await transaction.put({ [BROWSER_QUOTA_DATE_KEY]: today,
        [BROWSER_QUOTA_TOTAL_KEY]: aggregate + BROWSER_RUN_RESERVATION_MS });
      return { ok: true, reservation: { utcDate: today, chargedMs: BROWSER_RUN_RESERVATION_MS } };
    });
  } catch {
...
export async function settleBrowserRun(
  ...
  try {
    return await storage.transaction(async (transaction) => {
      ...
      const storedDate = stored.get(BROWSER_QUOTA_DATE_KEY);
      const storedTotal = stored.get(BROWSER_QUOTA_TOTAL_KEY);
      if (storedDate !== reservation.utcDate || !validAggregate(storedTotal)
        || storedTotal < reservation.chargedMs) return false;
      await transaction.put({ [BROWSER_QUOTA_DATE_KEY]: reservation.utcDate,
